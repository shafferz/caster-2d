...
class Core(object):
    def __init__(self):
        ...
        pg.mixer.music.load(
            os.path.join(MUSIC_DIR, self.song_list[self.song_list_head])
        )
        pg.mixer.music.play()
    ...
    def dispatch(self, event):
        ...
        # Handling mouse moving events, used to smooth the drawing strokes
            ...
            if self.draw_on:
                pg.draw.circle(self.canvas, BLACK, (adj_x, adj_y), 25)
                GameTools.roundline(
                    self.canvas, BLACK, (adj_x, adj_y), self.last_pos, 25
                )
                    ...
                    if self.player_turn:
                        ...
                        if self.spell_crafter.spell_ready(self.player.mana):
                            ...
                            if self.player_acted and self.enemy_acted:
                                ...
                                self.enemy_acted = False
                            self.player.eot()
                            self.opponent_bot.random_cast(min(self.enemy.max_mana, 5))
    ...
    def calculate_result(self):
        ...
        if self.spell_crafter.dominant_element[0] == self.opponent_bot.dominant_element[
            0
        ] - 1 or (
            self.spell_crafter.dominant_element[0] == 5
            and self.opponent_bot.dominant_element[0] == 1
        ):
            p1_has_adv = True
        # Then check if p2 has advantage
        if self.opponent_bot.dominant_element[0] == self.spell_crafter.dominant_element[
            0
        ] - 1 or (
            self.opponent_bot.dominant_element[0] == 5
            and self.spell_crafter.dominant_element[0] == 1
        ):
            p2_has_adv = True
    ...
    def render_overlay(self):
        ...
        if self.game_state == 1:
            ...
            self.game_font.render_to(
                self.screen, (330, 70), round_str, fgcolor=BLUEGREY, size=30
            )
            ...
            if self.player_turn:
                ...
                if self.spell_crafter.cost <= self.player.mana:
                    ...
                    self.game_font.render_to(
                        self.screen, (605, 230), cost_str, fgcolor=BLUEGREY, size=25
                    )
                elif self.spell_crafter.cost > self.player.mana:
                    cost_str = "Cost: " + str(self.spell_crafter.cost)
                    self.game_font.render_to(
                        self.screen, (605, 230), cost_str, fgcolor=RED, size=25
                    )
            mana_str = (
                "Mana: "
                + str(self.player.mana - self.spell_crafter.cost)
                + "/"
                + str(self.player.max_mana)
            )
            self.game_font.render_to(
                self.screen, (605, 255), mana_str, fgcolor=BLUE, size=25
            )
            hp_str = "HP: " + str(self.player.hitpoints) + "/100"
            self.game_font.render_to(
                self.screen, (605, 280), hp_str, fgcolor=RED, size=25
            )
            ems = (
                "Mana: "
                + str(self.enemy.mana - self.opponent_bot.cost)
                + "/"
                + str(self.enemy.max_mana)
            )
            self.game_font.render_to(
                self.screen, (605, 490), ems, fgcolor=BLUE, size=25
            )
        ...
        if self.game_state == 2:
            if self.alive == False:
                self.game_font.render_to(
                    self.screen, (20, 50), "You lose", fgcolor=RED, size=165
                )
                self.game_font.render_to(
                    self.screen,
                    (220, 275),
            ...
            if self.won == True:
                self.game_font.render_to(
                    self.screen, (40, 50), "Victory!", fgcolor=BLUEGREY, size=165
                )
                self.game_font.render_to(
                    self.screen,
                    (220, 275),
    ...
    def render_main_menu(self):
        ...
        if self.game_substate == 0:
            self.game_font.render_to(
                self.screen, (95, 60), TITLE, fgcolor=BLUEGREY, size=(120, 240)
            )
            self.game_font.render_to(
                self.screen, (97, 62), TITLE, fgcolor=BLUE, size=(120, 240)
            )
            # Display game options with rectangles associated for event handling
            self.game_font.render_to(
                self.screen, (293, 258), "Play", fgcolor=BLUEGREY, size=90
            )
            self.game_font.render_to(
                self.screen, (295, 260), "Play", fgcolor=BLUE, size=90
            )
            self.play_rect = pg.Rect(295, 260, 215, 90)
            self.game_font.render_to(
                self.screen, (213, 348), "Options", fgcolor=BLUEGREY, size=90
            )
            self.game_font.render_to(
                self.screen, (215, 350), "Options", fgcolor=BLUE, size=90
            )
        ...
        # Substate 2 is main menu play submenu
            ...
            self.game_font.render_to(
                self.screen, (203, 258), "Tutorial", fgcolor=BLUEGREY, size=90
            )
            self.game_font.render_to(
                self.screen, (205, 260), "Tutorial", fgcolor=BLUE, size=90
            )
            self.tut_rect = pg.Rect(105, 230, 485, 90)
            self.game_font.render_to(
                self.screen, (113, 348), "Multiplayer", fgcolor=BLUEGREY, size=90
            )
            self.game_font.render_to(
                self.screen, (115, 350), "Multiplayer", fgcolor=GREY, size=90
            )
    ...
    def render_player_game_screen(self):
        ...
        self.game_font.render_to(
            self.screen, (275, 515), "Press Tab to Clear", fgcolor=BLUEGREY, size=25
        )
        self.game_font.render_to(
            self.screen,
            (195, 540),
            "Press Space to Submit Drawing",
            fgcolor=BLUEGREY,
            size=25,
        )
        self.game_font.render_to(
            self.screen, (670, 70), "You", fgcolor=BLUEGREY, size=30
        )
        self.game_font.render_to(
            self.screen, (655, 460), "Enemy", fgcolor=BLUEGREY, size=30
        )
    ...
    def render_enemy_game_screen(self):
        ...
        self.game_font.render_to(
            self.screen,
            (200, 125),
            "The opponent's spell has the glyphs:",
            fgcolor=BLUEGREY,
            size=25,
        )
